...
def create_timecard_entry(dates, headers, login_response, project_name):
    for date in dates:
        ...
        try:
            response = requests.post(CREATE_TIMECARD_ENTRY_URL, headers=headers, data=urlencode(payload), cookies=login_response.cookies)
            if response.status_code == 200:
                print(f"Successful request for the date {formatted_date}")
            else:
                print(f"Request error for the date {formatted_date}. Status code: {response.status_code}")
...
def get_dates_from_excel(file_path, sheet_name, start_date, end_date):
    try:
        ...
        filtered_dates = df[(df['date'] >= start_date_parsed) & (df['date'] <= end_date_parsed)]
...
if __name__ == "__main__":
    ...
    parser.add_argument('--project', type=str, default='MTK', help="Name of the project (default: MTK)")
    parser.add_argument('--hours', type=int, default=8, help="Number of hours to load (default: 8)")
    parser.add_argument('--email', type=str, required=True, help="Email to login into Tick")
    parser.add_argument('--password', type=str, required=True, help="Password to login into Tick")
    parser.add_argument('--file', type=str, required=True, help="Path to the Excel file containing dates")
    parser.add_argument('--sheet', type=str, required=True, help="Sheet name representing the person")
    parser.add_argument('--start_date', type=str, required=True, help="Start date for filtering in dd/mm/yyyy format")
    parser.add_argument('--end_date', type=str, required=True, help="End date for filtering in dd/mm/yyyy format")
    args = parser.parse_args()
    
    if args.project not in project_name_to_id_map:
        print(f"Error: Unknown project '{args.project}'. Please provide a valid project. \nValid projects: ({','.join([project_name for project_name in project_name_to_id_map.keys()])})")
        exit(1)
    ...
    print(f'This script is going to load {args.hours} hours per day for the {args.project} project.')
